...
from use_dbn_on_bigset.sound import *
from use_dbn_on_bigset.audiodb import *
...
class Features:
	...
	def feature_plot(self, feature=None, normalize=False, dbscale=False, norm=False, interp='bicubic', labels=False):
		...
		if feature == 'stft':
			if not self._have_stft:
				...
				adb.feature_plot(pylab.absolute(self.STFT), normalize, dbscale, norm, title_string="STFT",
				                 interp=interp)
				if labels:
					self._feature_plot_xticks(
						pylab.linspace(0, self.STFT.shape[1], 10) * (self.feature_params['nhop'] / self.sample_rate))
					self._feature_plot_yticks(
						pylab.linspace(0, self.STFT.shape[0], 20) * (self.sample_rate / (self.feature_params['nfft'])))
	...
	def _chroma_hcqft(self):
		...
		if fp['verbosity']:
			...
			fp['lcoef'], fp['ncoef'], fp['intensify'])
		return True
