...
from use_dbn_on_bigset import error
import features
...
class adb:
    ...
    def get(dbname, mode="r"):
        ...
        db.configQuery = {'accumulation': 'track', 'distance': 'eucNorm', 'exhaustive': False, 'falsePositives': False,
                          'npoints': 10, 'ntracks': 10, 'seqStart': 100, 'seqLength': 10, 'radius': 0.4,
                          'absThres': -4.5, 'resFmt': 'list'}
    ...
    def insert_audio_files(fileList, dbName, chroma=True, mfcc=False, cqft=False, progress=None):
        ...
        for a, i in enumerate(fileList):
            ...
            if chroma:
                F.feature_params['feature'] = 'chroma'
            elif mfcc:
                F.feature_params['feature'] = 'mfcc'
            elif cqft:
                F.feature_params['feature'] = 'cqft'
            else:
    ...
    def insert_feature_files(featureList, powerList, keyList, dbName, delta_time=None, undo_log10=False):
        ...
        for feat, pwr, key in zip(featureList, powerList, keyList):
            ...
            if undo_log10:
                F = 10 ** F
            if delta_time != 0.0:
                T = pylab.c_[pylab.arange(0, a) * delta_time, (pylab.arange(0, a) + 1) * delta_time].reshape(1,
                                                                                                             2 * a).squeeze()
            else:
                T = None
